...
class AccessGroupSynchronizerSpec extends BaseSpec {
    ...
    "synchronization happens successfully" should {
      "indicate access groups were updated" in new TestScope {
        val accessGroups: Seq[AccessGroup] = Seq(
          buildAccessGroup(Some(Set(agentUser)), Some(Set(enrolmentVat, enrolmentPpt, enrolmentCgt)))
        )
        ...
        accessGroupSynchronizer
          .syncWithEacd(
            ...
            GroupDelegatedEnrolments(
              Seq(AssignedClient(enrolmentVat.service, enrolmentVat.identifiers, None, agentUser.id))
            ),
            agentUser
          )
  ...
  "Calculating user enrolments difference" when {
    ...
    "GroupDelegatedEnrolments contains fewer enrolments than in access group" should {
      "work" in new TestScope {
        ...
        val groupDelegatedEnrolments: GroupDelegatedEnrolments = GroupDelegatedEnrolments(
          Seq(
            AssignedClient(enrolmentVat.service, enrolmentVat.identifiers, None, agentUser.id)
          )
        )
        val enrolmentsToRemove: Set[UserEnrolment] = accessGroupSynchronizer
          .calculateRemovalSet(
            accessGroups,
            groupDelegatedEnrolments
          )

        enrolmentsToRemove shouldBe Set(
          UserEnrolment(
            agentUser.id,
  ...
  "Building AccessGroups With Updates" when {
    ...
    "removal set contains fewer enrolments than in access groups" should {
      "have corresponding enrolments and users removed" in new TestScope {
        ...
        val accessGroupsHavingRemovalsApplied: Seq[AccessGroup] =
          accessGroupSynchronizer.applyRemovalsOnAccessGroups(accessGroups, removalSet, agentUser).futureValue
    ...
    "removal set contains an unknown user id" should {
      "not affect existing team members of access group" in new TestScope {
        ...
        firstAccessGroupHavingRemovalsApplied.teamMembers shouldBe Some(Set(agentUser))
      }
    }
  }
  ...
  trait TestScope {
    ...
    val agentUser: AgentUser = AgentUser("userId", "userName")
    ...
    def buildAccessGroup(teamMembers: Option[Set[AgentUser]], clients: Option[Set[Enrolment]]): AccessGroup =
      AccessGroup(arn, groupName, now, now, agentUser, agentUser, teamMembers, clients)
